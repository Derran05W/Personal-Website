...
- [~] **Phase 35 — Height re-grade under the new camera + the one true eye-line constant** — 2026-07-26: first run terminated by the print-mode 600 s background-wait ceiling (since fixed in run-all-phases.sh) with gates green on partial work (eye-line constant + heightLaw.test.ts + placesLayer/cityPackScale edits, uncommitted in the tree) — RESUME, don't restart (see phase-35-plan.md if present)
