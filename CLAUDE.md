...
- [ ] **Phase 25.5 — asset pipeline & pack ingestion**: normalize pack into assets/city-pack/ (kebab-case), gltf-transform optimize/Draco, typed manifest w/ measured dims, loader seam (useGLTF+preload), InstancedMesh-from-GLB strategy (per-model instancing — 7 building types ≈ 7 draw calls not 570), lit-vs-unlit material verdict for pack textures under the blue-hour rig, scale calibration to car-derived units
- [ ] **Phase 25.6 — world re-dress**: car-derived road widths + thinner dashes; filler massing re-targeted to instanced pack buildings (district tint variation); pack Traffic Lights at correct per-intersection counts (emissive heads, cycling), Power Boxes, furniture rows (hydrants/benches/trash/trees/bus stops), Road Bits/manholes; parked pack vehicles as props
