...
- [!] **Phase 76 ⚑ — Camera v3: readability in the widened corridors** — **USER GATE** — 2026-08-07: lab shipped, `CAMERA` **untouched** (rig E still law; `camera.law.test.ts`'s rig-E literals green + unmodified; prod delta **+198 bytes / +0.005 %**, candidate rows + corridorLaw grep-proven tree-shaken out); **the corridor question turned out to be moot** — all 8 candidates measured **0.0 % eye-inside AND 0 % boresight at all 10 vantages, on every drive (842–1,346 frames each) and every ★3 chase**, including F/L at hr 18.0–18.5, far outside the inherited bound; new `world/toronto/corridorLaw.ts` re-derives the ceiling per class from `Street.halfWidth + SIDEWALK.widthWu` and **found the inherited `14.8` was never geometry** (it is preset B's own hr 14.838 rounded down — an empirical discriminator; its stated geometry cited a spine width Phase 27 had superseded SIX phases earlier, making it ~23 % more PERMISSIVE than the geometry live when measured, not conservative) + FINDING 2 (lane-referenced, not centreline: P75's tripled `LANE_OFFSET_WU` made worst-lane spine clearance 8.5→**7.95**, slightly WORSE, while the centreline ceiling rose 31 %) + FINDING 3, the headline: **since P35's streetwall cap the corridor is a SECONDARY constraint — the eye line is primary**, so a future gate treating `min(class ceilings)` as hard would reject rigs the world carries; also caught the ramp is **non-monotonic in tier** (worst hr 14.4281 at rest ★4, not the ★5 corner P34 recorded — sweep, don't sample); 8 candidates as DEV-gated preset data w/ shared-leaf overrides whose contamination risk is **designed out** (every apply writes every leaf as `override ?? DEFAULTS`, one tuple = both vocabulary and restore) and a table-wide `presetPitchMaxDeg ≤ 70` law (H/I got pitch-only re-grades to the shipped rig's own 69.5° saturated pose so their drive/chase footage is of a driveable rig — deviation from the plan, argued); readability instrumentation at **byte-identical prod cost** (A/B-proven; `@__PURE__` + niladic helper needed — DEV DCE alone was insufficient); **THE BATTERY CAUGHT ITS OWN HEADLINE METRIC LYING** — `cityInFrameFraction` read **100 % on a frame with no building in it** (near-plane-clipped corners at NDC ±1000 → bounding rect = whole screen), first run KILLED and re-run after a rewrite to true-silhouette clip-space rasterization now validated against an independent ray-traced oracle at **0.000 pp mean error over 150 real poses**; **THE FINDINGS**: rig E shows **0.0 % city at 4 of 10 vantages** (ny-centre 6.8 % = bare asphalt, vs I's 19.5 % full storefront row — the money pair), pitch trades city-in-frame in **opposite directions** open-vs-canyon (I wins junctions 3×–∞, E beats H/J *inside* canyons 56.4 vs 48.6, L wins canyons at 79.7) so **no rig maximises both**, and on the shipped rig **a ★3 chase with 6 live police put a pursuer on screen for exactly ZERO of ~950 frames** — pinned as geometry (on-screen only within **26.25 m** best-bearing / 11.3 m camera-side; 37 m is off-screen on all 72 bearings) while **J alone scores 1.02 mean / max 4** (its single changed leaf is `speedZoom` 4→8: what shows you the cops is how much the frame opens **when you are moving**, not pitch, lens or look-ahead — K contributes literally nothing); widest frustum costs only +6 calls / +4.3 % tris (high); P38 verdicts re-measured **geometrically** (no candidate puts horizon in frame; H/I/J steeper ⇒ hold a fortiori; **G/L/F owe an empirical NDC re-measure if picked**); 4,268 tests (+137), smoke 31/31, bench ×3 green (low **99,145/120,000**), 0 errors. **RECOMMENDATION: J** (only rig that solves the chase, safer corridor than E, un-binds the streetwall cap so its city numbers are a lower bound) · **L** if city-look outweighs seeing cops · **G** for the smallest possible change. **AWAITING USER: pick the rig** (keeping E is valid) (see phase-76-notes.md)
